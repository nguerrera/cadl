...
function createOAPIEmitter(program: Program, options: ResolvedOpenAPI3EmitterOptions) {
    ...
    for (const data of response.responses) {
      if (data.headers.size > 0) {
        ...
        for (const [key, value] of data.headers) {
          ...
          openapiResponse.headers[key] = getResponseHeader(value.property, value.type);
        }
      }
  ...
  function getResponseHeader(prop: ModelTypeProperty, type: Type) {
    const header: any = {};
    populateParameter(header, prop, type, "header");
    delete header.in;
    delete header.name;
  ...
  function emitEndpointParameters(parameters: HttpOperationParameter[]) {
    for (const { name, kind, property: property, type } of parameters) {
      // If param is a global parameter, just skip it
      if (params.has(property)) {
        currentEndpoint.parameters.push(params.get(property));
        continue;
      }

      switch (kind) {
        case "path":
          emitParameter(property, type, "path");
          break;
        case "query":
          emitParameter(property, type, "query");
          break;
        case "header":
          if (name !== "content-type") {
            emitParameter(property, type, "header");
          }
          break;
      }
    }
  ...
  function emitRequestBody(
    ...
    const contentTypeParam = parameters.parameters.find(
      (p) => p.kind === "header" && p.name === "content-type"
    );
    const contentTypes = contentTypeParam
      ? ignoreDiagnostics(getContentTypes(contentTypeParam.property))
      : ["application/json"];
  ...
  function emitParameter(param: ModelTypeProperty, type: Type, kind: OpenAPI3ParameterType) {
    const ph = getParamPlaceholder(param);
    currentEndpoint.parameters.push(ph);

    // If the parameter already has a $ref, don't bother populating it
    if (!("$ref" in ph)) {
      populateParameter(ph, param, type, kind);
    }
  }
